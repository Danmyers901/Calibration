...
!!    temperature exceeds 0 degrees Celcius and rain-on-snow events
...
!! elevation bands
     ...
     &                                             - sub_smtmp(ib,isub))
		  ...
		  !! Calculate snowmelt from rain-on-snow events and add to smleb
		  !! (Added by DM 6/18/2020)
		  if (snoeb(ib,j) > 0 .and. tavband(ib,j) > sub_sftmp(ib,isub) .and.
     &		  pcpband(ib,j) > 0) then
			smleb = smleb + 6.12E-10 * 24. * ((tavband(ib,j) + 273.) ** 4. -
     &		273. ** 4.) + 0.0125 * pcpband(ib,j) * 1 * 
     &		max(tavband(ib,j), 0.) + 8.5 * 0.15 * (24. / 6.) * ((0.9 *
     &		2.7489E8 * exp(-4278.63 / (tavband(ib,j) + 242.792)) - 6.11) +
     &		0.00057 * 33.86 * (29.9 - 0.335 * elevb(ib,isub) + 0.00022 * 
     &		elevb(ib,isub) ** 2.4) * tavband(ib,j))
		  endif			  
...
!! no elevation bands
	...
	ib = 1
     ...
     &       (sub_smfmx(ib,isub) - sub_smfmn(ib,isub)) / 2.    !! 365/2pi = 58.09
		  ...
		  !! Calculate snowmelt from rain-on-snow events and add to snomlt
		  !! (Added by DM 6/18/2020)
		  if (sno_hru(j) > 0 .and. tmpav(j) > sub_sftmp(ib,isub) .and. 
     &		  precipday > 0) then
			snomlt = snomlt + 6.12E-10 * 24. * ((tmpav(j) + 273.) ** 4. - 273. **
     &		4.) + 0.0125 * precipday * 1 * max(tmpav(j), 0.) + 8.5 * 0.15 *
     &		(24. / 6.) * ((0.9 * 2.7489E8 * exp(-4278.63 / (tmpav(j) + 
     &		242.792)) - 6.11) + 0.00057 * 33.86 * (29.9 - 0.335 * 200. + 
     &		0.00022 * 200. ** 2.4) * tmpav(j))
		  endif
								
		  !! Notes (DM 6/18/2020)
			!! I emailed myself the references for UADJ options. 0.15 is selected from USGS
			!!     https://pubs.usgs.gov/sir/2006/5318/data/snowpck_dpm.htm but they all are
			!!     in the 0.03 to 0.20 range. 
			!! I set the subbasin elevation to 200 m (typical for the Great Lakes Basin) but
			!!     need to find a way to bring in actual subbasin elevation.
			
			
			
		  
		  !! Finish snowmelt calculations
