function [newOF,newCg,POSITION] = CompOF(x,AMALGAMPar,Extra) % modified by DM 9/23/20
% This function computes the objective function for each x value
% It has been modified for multivariable calibration.
% It has also been modified by DM 9/23/20 to output the orderly row position.
...
parfor iRun=1:AMALGAMPar.N
    ...
    [OF1(iRun), OF2(iRun), OF3(iRun), OF4(iRun), POSITION(iRun)] = mcalib(iRun,x(iRun,:),Extra) % Added OF3, OF4 here (DM 6/11/2020). Modified by DM 9/23/20.
end
